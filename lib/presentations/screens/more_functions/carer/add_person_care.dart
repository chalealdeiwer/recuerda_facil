...
class _SharedNotesScreenState extends ConsumerState<AddPersonCareScreen> {
  ...
  Future<void> scanQr() async {
    if (await requestCameraPermission()) {
      ...
      showAlertDialogCustom("Información", "No se concedieron permisos para la Cámara ");
    }
  }
  ...
  Future<void> scannerTrue() async {
    try {
      ...
      if (cameraScanResult != null) {
        if (cameraScanResult.toString() ==
            FirebaseAuth.instance.currentUser!.uid.toString()) {
              showAlertDialogCustom("Información", "No se puede escanear usted mismo");
          setState(() {
          ...
          if (await searchUserUid(cameraScanResult.toString())) {
            ...
            if(await addUserCarer(FirebaseAuth.instance.currentUser!.uid, qrValue) ){
            showAlertDialogCustom("Información", "El usuario se ha encontrado y se asigno a personas a cuidar");
            context.push('/carer_list/$qrValue');
          
            }else{
            showAlertDialogCustom("Información", "El usuario ya se encuentra en sus personas a cuidar");

            }
          }else{
            showAlertDialogCustom("Información", "El usuario NO se ha encontrado");
          }
        }
      } else {
      ...
      showAlertDialogCustom("Error", "No se pudo escanear, intenta de nuevo: $e");
    }
  }
